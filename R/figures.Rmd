...
```{r Figure2-Pop-Comp-Inno, fig.dim=c(8,8)}
...
custom_labels.y <- c(expression(atop("20","(e"^3*")")),
                     expression(atop("55","(e"^4*")")),
                     expression(atop("148","(e"^5*")")),
                     expression(atop("403","(e"^6*")")))
custom_breaks.y <- round(exp(seq(from = 3,to = 6, by = 1)), digits = 0)
...
p <- ggplot(data_inno_mean_plot,  
  ...
  geom_signif(comparisons = list(c("Lower Aripo", "Upper Aripo")),  # significance brackets 
              ...
              y_position = 6.3, color = "black", size = 1,
              textsize = 12, fontface = "bold") +
  xlab("Population") +
  ylab(expression(paste(bold(Goal~zone~latency)~bold((sec))~bold(""%+-%"")*bold("95"*"%"~C.I.)))) +
...
```{r Figure3-Inno-Predict-Total-Zones, fig.dim=c(8,6)}
...
p <- ggplot(data_inno_mean_plot,
  ...
  ylab(expression(paste(bold(Goal~zone~latency)~bold((sec))))) +
  scale_color_manual(values = c("#E41A1C", "#377EB8","#377EB8"),
                     name = "Population") +
  scale_shape_discrete(name = "Population") + 
